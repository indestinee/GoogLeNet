...
class GoogLeNet(object):
    def __init__(self, args):
        ...
        if os.path.exists("%s/%s.index" % (model_path, 'model')):
            self.model.load("%s/%s" % (model_path, 'model'))
            print("load existing checkpoint from %s" % self.model_name)

    def fit(self, X, Y, n_epoch=100):
        self.model.fit(X, Y, n_epoch=n_epoch, validation_set=0.1, shuffle=True, show_metric=True, batch_size=32, snapshot_step=200, snapshot_epoch=True, run_id=self.model_name)
